    ...
    for (int64 b = 0; b < batch_size; ++b) {
      ...
      for (int64 bb = 0; bb < num_boxes; ++bb) {
        ...
        if (min_box_row >= 0) {
          for (int64 j = min_box_col_clamp; j <= max_box_col_clamp; ++j)
            for (int64 c = 0; c < depth; c++) {
              canvas(b, min_box_row, (ptrdiff_t)j, (ptrdiff_t)c) =
        ...
        if (max_box_row < height) {
          for (int64 j = min_box_col_clamp; j <= max_box_col_clamp; ++j)
            for (int64 c = 0; c < depth; c++) {
              canvas(b, max_box_row, (ptrdiff_t)j, (ptrdiff_t)c) =
                  static_cast<T>(color_table[color_index][c]);
            }
        }
        // Draw left line.
        if (min_box_col >= 0) {
          for (int64 i = min_box_row_clamp; i <= max_box_row_clamp; ++i)
            for (int64 c = 0; c < depth; c++) {
              canvas(b, i, (ptrdiff_t)min_box_col, (ptrdiff_t)c) =
                  static_cast<T>(color_table[color_index][c]);
            }
        }
        // Draw right line.
        if (max_box_col < width) {
          for (int64 i = min_box_row_clamp; i <= max_box_row_clamp; ++i)
            for (int64 c = 0; c < depth; c++) {
              canvas(b, i, (ptrdiff_t)max_box_col, (ptrdiff_t)c) =
                  static_cast<T>(color_table[color_index][c]);
            }
        }
      }
